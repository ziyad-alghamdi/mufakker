...
export default function AdminRegistrations() {
  ...
  // ✅ تحميل البيانات مباشرة من جدول registrations
 async function loadRegistrations() {
  ...
  await supabase
    ...
    .eq("id", id);
// 2️⃣ مناداة API الإيميل (الملف الجديد)
 ...
 /* const res = await fetch("/api/generate-certificate", {
    ...
    body: JSON.stringify({  
      ...
      workshop_id: regData.workshop_id,
  ...
  return (
    <div className="page">
      ...
      <div className="content">
        <h1 className="title">طلبات التسجيل في الورش</h1>
          ...
          <div className="list">
            {registrations.map((r) => (
              <div className="card" key={r.id}>
                ...
                {/* 🟦 بيانات الورشة */}
                  <h3>📌 الورشة: {r.workshops?.title || "—"}</h3>
                  <p>📅 التاريخ: {r.workshops?.date || "—"}</p>
                ...
                {r.status === "pending" ? (
                  <div className="actions">
                    <button className="approve" onClick={() => approve(r.id)}>✔ قبول</button>
                    <button className="reject" onClick={() => reject(r.id)}>✖ رفض</button>
                  </div>
                ) : r.status === "approved" ? (
